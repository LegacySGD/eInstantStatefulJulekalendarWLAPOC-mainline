...
public class StatefulJulekalendarWLAPOCEngineManagerTest
{
	...
	{
		logger = Logger.getLogger(StatefulJulekalendarWLAPOCEngineManagerTest.class);
		gameParams = XMLUtil.parseResource("paymodel/StatefulJulekalendarWLAPOCIW/GameParams.xml").getDocumentElement();
		previousOutcome = XMLUtil.parseResource("paymodel/StatefulJulekalendarWLAPOCIW/InitOutcome.xml").getDocumentElement();
			...
			throws Exception
	{
		...
		playerInput = new PlayerInputBuilder()
				...
				.appendWagerInput(new Wager("Wager1", 50))
				.appendJSONInput("{'testData':0}").build();
		TestODEProxy.setJsonResponse("{'prizeValue':0,'moreStuff':'toTest','prettyPrint':'forGhst'}");
		Node outcome = manager.play(InputUtil.buildInput(gameParams, previousOutcome, playerInput));
		logger.debug(outcome);
		...
		assertEquals("Wager", JXPathUtil.getString(outcome, "//NextStage/text()"));
		assertSettledPendingPayout(outcome, 50, 0, 0);
	}
			...
			throws Exception
	{
		...
		assertEquals("Wager", JXPathUtil.getString(outcome, "//NextStage/text()"));
		assertSettledPendingPayout(outcome, 50, 0, 100);
	}
